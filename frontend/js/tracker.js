...
function icon(event, facing) {
  ...
  return L.divIcon({
    ...
    className: `ao-pin${loud ? ` is-loud is-${event.kind}` : ''}`
      ...
      // An inference from a warning rather than a sighting, and drawn as
      // such: the same reduced weight an advisory gets, for the same reason.
      + (event.from_warning ? ' is-advisory is-from-warning' : '')
...
function popup(event) {
  ...
  if (event.from_warning) {
    // Said in full, because the mark looks like an ordinary courseless drone
    // and the difference between "a drone was seen here" and "a warning about
    // drones was declared over this region" is the whole of what it means.
    rows.push('<b>From a warning, not a sighting.</b> A warning about drones '
      + 'was declared over this region; nobody reported one at this point. '
      + 'The position is the middle of the region, there is no course, and '
      + 'nothing here is extrapolated from it.');
  }
  if (event.area_only) {
 ...
 *
 * A drone raised from a warning is squarely the second kind, and it is the
 * exact picture described above: it is created at its warning's own position,
 * so without this the two land on the same pixel every single time.
 */
export const floats = (event) => event.kind === 'alert'
  || Boolean(event.area_only)
  || Boolean(event.from_warning)
 ...
 *
 * A drone raised FROM a warning sits at a region's centre too, and IS drawn
 * -- which looks like an exception here and is not one. It is exempt because
 * of what it is not marked as: _drone_from_warning sets neither area_only nor
 * region_scope, deliberately and with the reason written there, so it never
 * reaches either test below. An exemption spelt out here as well would be a
 * second rule saying what the first already says, and the kind that goes
 * stale silently when the other one moves.
 */
export const regionOnly = (event) => event.kind !== 'alert'
